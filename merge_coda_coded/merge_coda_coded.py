...
if __name__ == "__main__":
    ...
    parser.add_argument("raw_column", metavar="raw-column", help="Name of raw input column to apply code to", nargs=1)
    parser.add_argument("coded_column", metavar="coded-column", help="Name of column to store code in", nargs=1)
    parser.add_argument("input", help="Path to input file, containing a list of TracedData objects as JSON", nargs=1)
    parser.add_argument("coda_input", metavar="coda-input", help="TODO", nargs=1)
    ...
    with open(coda_input_path, "rb") as f:
        data = list(TracedDataCodaIO.import_coda_to_traced_data_iterable(data, col, col_coded, f))
